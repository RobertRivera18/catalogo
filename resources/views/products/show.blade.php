        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14">
            ...
            <div class="lg:sticky lg:top-6 self-start">
                <div class="relative bg-white rounded-2xl border border-gray-100 shadow-sm p-3">
                    ...
                    {{-- Badge descuento — dentro de la imagen --}}
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-5 border-b border-gray-100">
                        ...
                        <div class="inline-flex items-center gap-2 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-full">
                            <div class="flex items-center gap-0.5">
                                @for ($i = 1; $i <= 5; $i++)
                                    <svg class="w-3 h-3 {{ $avg >= $i ? 'text-amber-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <span class="text-xs text-gray-400">({{ $count }} {{ $count === 1 ? 'reseña' : 'reseñas' }})</span>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-sm">
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr>
                                    ...
                                    <td class="py-2.5 text-gray-900 font-medium text-right capitalize">{{ $product->brand->name }}</td>
                                </tr>
                                <tr>
                                    <td class="py-2.5 pr-4 text-gray-400">Categoría</td>
                                    <td class="py-2.5 text-gray-900 font-medium text-right">{{ $product->subcategory->category->name }}</td>
                                </tr>
                                ...
                                @if ($product->specifications->count())
                                    ...
                                    @foreach ($product->specifications as $spec)
                                        <tr>
                                            ...
                                            <td class="py-2.5 text-gray-900 font-medium text-right">{{ $spec->value }}</td>
                                        </tr>
                                    @endforeach
                                @endif
                            </tbody>
                            ...
                            @foreach ($product->includedAccessories as $acc)
                                <div class="flex flex-col items-center gap-2 w-20 group">
                                    <div class="w-16 h-16 rounded-xl border-2 border-emerald-200 bg-gray-50 flex items-center justify-center overflow-hidden group-hover:border-emerald-400 transition-colors">
                                        @if ($acc->image)
                                            <img src="{{ Storage::url($acc->image) }}" alt="{{ $acc->name }}" class="w-full h-full object-contain p-1">
                                        @else

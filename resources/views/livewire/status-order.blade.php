        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 font-bold px-4 py-2 rounded-xl border-2 border-blue-200">
            ...
            <div class="flex items-center gap-2">
                @if($order->status == 5)
                    <span class="inline-flex items-center gap-2 bg-red-100 text-red-700 font-bold px-5 py-3 rounded-xl border-2 border-red-300">
                        <i class="fas fa-times-circle"></i>
                        Anulado
                    </span>
                @elseif($order->status == 4)
                    <span class="inline-flex items-center gap-2 bg-green-100 text-green-700 font-bold px-5 py-3 rounded-xl border-2 border-green-300">
                        <i class="fas fa-check-circle"></i>
                        Entregado
                    </span>
                @elseif($order->status == 3)
                    <span class="inline-flex items-center gap-2 bg-blue-100 text-blue-700 font-bold px-5 py-3 rounded-xl border-2 border-blue-300">
                        <i class="fas fa-shipping-fast"></i>
                        En Camino
                    </span>
                @else
                    <span class="inline-flex items-center gap-2 bg-yellow-100 text-yellow-700 font-bold px-5 py-3 rounded-xl border-2 border-yellow-300">
    ...
    <div class="bg-white rounded-2xl shadow-xl border-2 border-gray-100 p-8 md:p-12 mb-8">
        <div class="relative flex items-center justify-between">
            ...
            <div class="relative flex flex-col items-center flex-1">
                <div class="relative z-10 flex items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-full transition-all duration-500 {{ ($order->status >= 2 && $order->status != 5) ? 'bg-gradient-to-br from-blue-500 to-blue-600 shadow-lg scale-110' : 'bg-gray-300' }}">
                ...
                <div class="absolute -bottom-10 md:-bottom-12 left-1/2 -translate-x-1/2 whitespace-nowrap">
                    <p class="text-xs md:text-sm font-bold {{ ($order->status >= 2 && $order->status != 5) ? 'text-blue-600' : 'text-gray-500' }}">
                        Recibido
                    </p>
            ...
            {{-- Línea de conexión 1 --}}
            <div class="flex-1 h-2 mx-2 rounded-full transition-all duration-500 {{ ($order->status >= 3 && $order->status != 5) ? 'bg-gradient-to-r from-blue-500 to-blue-600' : 'bg-gray-300' }}">
            </div>

            {{-- Paso 2: Enviado --}}
            <div class="relative flex flex-col items-center flex-1">
                <div class="relative z-10 flex items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-full transition-all duration-500 {{ ($order->status >= 3 && $order->status != 5) ? 'bg-gradient-to-br from-blue-500 to-blue-600 shadow-lg scale-110' : 'bg-gray-300' }}">
                    <i class="fas fa-shipping-fast text-white text-xl md:text-2xl {{ ($order->status == 3 && $order->status != 5) ? 'animate-pulse' : '' }}"></i>
                </div>
                <div class="absolute -bottom-10 md:-bottom-12 left-1/2 -translate-x-1/2 whitespace-nowrap">
                    <p class="text-xs md:text-sm font-bold {{ ($order->status >= 3 && $order->status != 5) ? 'text-blue-600' : 'text-gray-500' }}">
                        Enviado
                    </p>
            ...
            {{-- Línea de conexión 2 --}}
            <div class="flex-1 h-2 mx-2 rounded-full transition-all duration-500 {{ ($order->status >= 4 && $order->status != 5) ? 'bg-gradient-to-r from-blue-500 to-green-500' : 'bg-gray-300' }}">
            </div>

            {{-- Paso 3: Entregado --}}
            <div class="relative flex flex-col items-center flex-1">
                <div class="relative z-10 flex items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-full transition-all duration-500 {{ ($order->status >= 4 && $order->status != 5) ? 'bg-gradient-to-br from-green-500 to-green-600 shadow-lg scale-110' : 'bg-gray-300' }}">
                    <i class="fas fa-gift text-white text-xl md:text-2xl"></i>
                </div>
                <div class="absolute -bottom-10 md:-bottom-12 left-1/2 -translate-x-1/2 whitespace-nowrap">
                    <p class="text-xs md:text-sm font-bold {{ ($order->status >= 4 && $order->status != 5) ? 'text-green-600' : 'text-gray-500' }}">
                        Entregado
                    </p>
        ...
        <form wire:submit.prevent="update">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <label class="relative cursor-pointer group">
                    ...
                    <div class="flex items-center gap-3 p-4 rounded-xl border-2 border-gray-300 peer-checked:border-blue-500 peer-checked:bg-blue-50 transition-all hover:border-gray-400 hover:shadow-md">
                        <div class="w-5 h-5 rounded-full border-2 border-gray-400 peer-checked:border-blue-500 peer-checked:bg-blue-500 flex items-center justify-center transition-all">
                ...
                <label class="relative cursor-pointer group">
                    ...
                    <div class="flex items-center gap-3 p-4 rounded-xl border-2 border-gray-300 peer-checked:border-green-500 peer-checked:bg-green-50 transition-all hover:border-gray-400 hover:shadow-md">
                        <div class="w-5 h-5 rounded-full border-2 border-gray-400 peer-checked:border-green-500 peer-checked:bg-green-500 flex items-center justify-center transition-all">
                            <i class="fas fa-check text-white text-xs opacity-0 peer-checked:opacity-100"></i>
                        </div>
                        <div class="flex-1">
                ...
                <label class="relative cursor-pointer group">
                    ...
                    <div class="flex items-center gap-3 p-4 rounded-xl border-2 border-gray-300 peer-checked:border-red-500 peer-checked:bg-red-50 transition-all hover:border-gray-400 hover:shadow-md">
                        <div class="w-5 h-5 rounded-full border-2 border-gray-400 peer-checked:border-red-500 peer-checked:bg-red-500 flex items-center justify-center transition-all">
                            <i class="fas fa-check text-white text-xs opacity-0 peer-checked:opacity-100"></i>
                        </div>
                        <div class="flex-1">
        ...
        <div class="bg-white rounded-2xl shadow-xl border-2 border-gray-100 p-6">
            <div class="flex items-center gap-3 mb-6">
                <div class="w-12 h-12 bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl flex items-center justify-center shadow-md">
                    <i class="fas {{ $order->envio_type == 1 ? 'fa-store' : 'fa-shipping-fast' }} text-white text-xl"></i>
                </div>
            ...
            <div class="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl p-5 border-2 border-purple-200">
                <div class="space-y-4">
                    <div class="bg-white rounded-lg p-4 border border-purple-200">
                        <p class="text-xs text-gray-500 mb-1 uppercase tracking-wide font-semibold">Persona que recibe</p>
                    ...
                    <div class="bg-white rounded-lg p-4 border border-purple-200">
                        <p class="text-xs text-gray-500 mb-1 uppercase tracking-wide font-semibold">Teléfono de contacto</p>
                        <a href="tel:{{ $order->phone }}" class="font-bold text-gray-900 text-lg hover:text-purple-600 transition-colors">
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach($items as $item)
                        <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <article class="flex-1">
                                        ...
                                        <div class="flex items-center gap-2 text-xs">
                                            @isset($item->options->color)
                                                <span class="inline-flex items-center gap-1 bg-gray-100 px-2 py-1 rounded-md">
                            ...
                            <td class="text-center py-4 px-2">
                                <span class="inline-flex items-center justify-center w-10 h-10 bg-blue-100 text-blue-700 font-bold rounded-lg">
                                    {{ $item->qty }}
                                </span>
                            </td>
                            <td class="text-right py-4 px-2">
                                <span class="font-black text-gray-900 text-lg">${{ number_format($item->price * $item->qty, 2) }}</span>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
                    ...
                    <tr class="border-t-2 border-gray-300">
                        ...
                        <td class="text-right py-6 px-2">
                            <span class="text-3xl font-black text-green-600">
                                ${{ number_format($items->sum(fn($item) => $item->price * $item->qty), 2) }}
                            </span>
                        </td>
                    </tr>
                </tfoot>
